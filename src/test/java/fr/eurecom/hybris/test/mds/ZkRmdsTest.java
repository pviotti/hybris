...
public class ZkRmdsTest extends HybrisAbstractTest {

    private static ZkRmds rmds;
    ...
    public static void beforeClassSetup() throws Exception {
        ...
        rmds = new ZkRmds(zkTestingServer.getConnectString(), MDS_TEST_ROOT, true);
    }

    // Executed before each test
    @Before
    public void setUp() throws Exception {
        rmds.emptyMetadataContainer();
        rmds.emptyStaleAndOrphansContainers();
    }
    ...
    public void testBasicWriteAndRead() throws HybrisException {
        ...
        rmds.tsWrite(key, md, ZkRmds.NONODE);

        Stat stat = new Stat();
        md = rmds.tsRead(key, stat);
        ...
        rmds.delete(key, tombstone, stat.getVersion());
        Stat newStat = new Stat();
        md = rmds.tsRead(key, newStat);
        assertNotNull(md);
    ...
    public void testOverwrite() throws HybrisException {
        ...
        rmds.tsWrite(key, new Metadata(new Timestamp(0, cid1), hash, 0, replicas, null), -1);  // znode does not exist, create hver. 0, zkver. 0
        rmds.tsWrite(key, new Metadata(new Timestamp(0, cid2), hash, 0, replicas, null), -1);  // NODEEXISTS retries because AAA > ZZZ, write hver. 0, zkver. 1

        retrieved = rmds.tsRead(key, stat);
        assertEquals(0, retrieved.getTs().getNum());
        assertEquals(cid2, retrieved.getTs().getCid());
        assertEquals(1, stat.getVersion());

        rmds.tsWrite(key, new Metadata(new Timestamp(1, cid1), hash, 1, replicas, null), 1);   // write hver. 1, zkver. 2

        rmds.tsWrite(key, new Metadata(new Timestamp(2, cid1), hash, 2, replicas, null), 2);   // write hver. 2, zkver. 3
        try {
            rmds.tsWrite(key, new Metadata(new Timestamp(2, cid1), hash, 3, replicas, null), 2);   // BADVERSION, fails because cids are equals
        } catch(HybrisException e) {
            e.printStackTrace();
            fail(); // TODO modify to test the new version which does not throw an exception
        }
        rmds.tsWrite(key, new Metadata(new Timestamp(2, cid2), hash, 4, replicas, null), 2);       // BADVERSION, retries because AAA > ZZZ, write hver. 2, zkver. 4
        ...
        try{
            rmds.tsWrite(key, new Metadata(new Timestamp(0, cid1), hash, 5, replicas, null), 0);  // BADVERSION, fails because hver is smaller
        } catch(HybrisException e) {
            e.printStackTrace();
            fail(); // TODO see above
        }

        rmds.tsWrite(key, new Metadata(new Timestamp(3, cid1), hash, 6, replicas, null), 1);  // BADVERSION, retries because 3 > 2, write hver. 3, zkver. 5
    ...
    public void testTransactionalWrite() {
        ...
        try {
            rmds.tsMultiWrite(map, statMap);
            for (Entry<String, Metadata> entry : map.entrySet()) {
                retrieved = rmds.tsRead(entry.getKey(), new Stat());
    ...
    public void testTransactionalRead() {
        ...
        try {
            LinkedHashMap<String, Metadata> res = rmds.tsMultiRead(statMap);
    ...
    public void testDeleteNotExistingKey() {
        ...
        try {
            ...
            rmds.delete(key, Metadata.getTombstone(ts1), -1);     // writes the tombstone no matter which znode version
            rmds.delete(key, Metadata.getTombstone(ts2), 5);      // finds a smaller znode version, retries (because ts is greater) and eventually succeeds
            rmds.delete(key, Metadata.getTombstone(ts3), 5);      // finds a smaller znode version, silently fails

            Stat stat = new Stat();
            Metadata md = rmds.tsRead(key, stat);
            assertNotNull(md);
        ...
        try {
            rmds.delete(key, Metadata.getTombstone(new Timestamp(1, "QWERTY")), 7);     // writes the tombstone anyway
            fail();
        } catch (HybrisException e) {
            KeeperException ke = (KeeperException) e.getCause();
    ...
    public void testTombstoneTimestamp() {
        ...
        try {
            int n = 0;
            rmds.tsWrite(key, new Metadata(new Timestamp(n, cid1), hash, 0, replicas, null), -1);      // znode does not exist, create hver. 0, zkver. 0

            n++;
            rmds.delete(key, Metadata.getTombstone(new Timestamp(n, cid1)), 0);

            Stat stat = new Stat();
            Metadata readMd = rmds.tsRead(key, stat);
    ...
    public void testSerialization() throws HybrisException {
        ...
        rmds.delete(key, Metadata.getTombstone(ts), stat.getVersion());
        assertTrue(rmds.tsRead(key, null).isTombstone());
    }
    ...
    public void testList() throws HybrisException {
        ...
        rmds.tsWrite(key1, md, ZkRmds.NONODE);
        rmds.tsWrite(key2, md, ZkRmds.NONODE);
        rmds.tsWrite(key3, md, ZkRmds.NONODE);
        rmds.tsWrite(key4, md, ZkRmds.NONODE);
        rmds.tsWrite(key5, md, ZkRmds.NONODE);

        List<String> listedKeys = rmds.list();
        ...
        rmds.tsWrite(key4, md, ZkRmds.NONODE);  // overwrites a key
        listedKeys = rmds.list();
        assertEquals(5, listedKeys.size());
        Stat stat = new Stat();
        Metadata newMd = rmds.tsRead(key4, stat);
        assertFalse(Arrays.equals(newMd.getHash(), hash));
        assertEquals(1, stat.getVersion());

        ts.inc("clientXYZ");
        rmds.delete(key3, Metadata.getTombstone(ts), stat.getVersion());      // remove a key
        assertTrue(rmds.tsRead(key3, null).isTombstone());
        listedKeys = rmds.list();
        ...
        for (String k : keys)                               // remove all keys
            rmds.delete(k, Metadata.getTombstone(tsd), -1);
        listedKeys = rmds.list();
    ...
    public void testGetAll() throws HybrisException {
        ...
        Map<String, Metadata> allMd = rmds.getAll();
        ...
        rmds.tsWrite(key4, md4, ZkRmds.NONODE);  // overwrites a key
        allMd = rmds.getAll();
        assertEquals(5, allMd.size());
        Stat stat = new Stat();
        Metadata newMd = rmds.tsRead(key4, stat);
        ...
        rmds.delete(key3, Metadata.getTombstone(newts), stat.getVersion());      // remove a key
        assertTrue(rmds.tsRead(key3, null).isTombstone());
        allMd = rmds.getAll();
    ...
    @Test
    public void testRawMdApi() throws HybrisException {
    	String key1 = this.TEST_KEY_PREFIX + new BigInteger(50, this.random).toString(32);
        String key2 = this.TEST_KEY_PREFIX + new BigInteger(50, this.random).toString(32);
        String key3 = this.TEST_KEY_PREFIX + new BigInteger(50, this.random).toString(32);
        byte[] value1 = new byte[20];
        byte[] value2 = new byte[20];
        byte[] value3 = new byte[20];
        this.random.nextBytes(value1);
        this.random.nextBytes(value2);
        this.random.nextBytes(value3);
        
        rmds.rawWrite(key1, value1);
        rmds.rawWrite(key2, value2);
        rmds.rawWrite(key3, value3);
        
        assertArrayEquals(value1, rmds.rawRead(key1));
        assertArrayEquals(value2, rmds.rawRead(key2));
        assertArrayEquals(value3, rmds.rawRead(key3));
        
        List<String> keys = rmds.rawList();
